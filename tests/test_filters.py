...
def test_bilateral_filter():
    ...
    filtered = filters.apply_3d_bilateral(
        noisy_data, sigma_spatial=2.0, sigma_color=0.1
    )

    assert filtered.shape == data.shape

    # Check Range Preservation
    assert np.max(filtered) > 1000.0, (
        f"Max value {np.max(filtered)} implies data was not denormalized!"
    )
