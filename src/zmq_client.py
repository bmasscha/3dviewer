...
class ViewerZMQClient(QObject):
    ...
    def __init__(
        ...
        inbound_port: int = 50001,
        outbound_port: int = 50001,
    ):
        """
        ...
        self.client = AcquilaClient(
            server_ip=server_ip, inbound_port=inbound_port, outbound_port=outbound_port
        )
        self.zmq_thread: threading.Thread | None = None
        self.running = False
        self.on_command_callback: Callable | None = None

        # Thread-safe queue for outgoing messages (Feedback/ACKs)
        self.outgoing_queue = queue.Queue()

        logger.info(
            f"ZMQ Client initialized (server: {server_ip}:{inbound_port}/{outbound_port})"
        )
    ...
    def _handle_command(self, client, command_data: dict) -> str:
        ...
        try:
            ...
            if cmd in ["load data", "load"]:
                cmd = "load_data"
            if cmd in ["set tf", "set_tf", "set transfer function"]:
                cmd = "set_transfer_function"
            ...
            if cmd == "set_transfer_function":
                ...
                print(
                    f"[ZMQ-CLIENT] Requesting set_transfer_function on MAIN THREAD: {arg1}"
                )
    ...
    def start(self, component_name: str = "3dviewer", physical_name: str = "3dviewer"):
        ...
        def run_zmq():
            ...
            self.running = True
            print(
                f"[ZMQ] Client Loop Started. Listening: {sub_port}, Sending: {pub_port}"
            )
        ...
        self.zmq_thread = threading.Thread(
            target=run_zmq, daemon=True, name="ZMQ-Client-Thread"
        )
        self.zmq_thread.start()

        logger.info(
            f"ZMQ client started (component: {component_name}, physical: {physical_name})"
        )
        print(
            f"[ZMQ-CLIENT] Client is now LISTENING on physical_name='{physical_name}'"
        )
    ...
    def is_running(self) -> bool:
        ...
        return (
            self.running and self.zmq_thread is not None and self.zmq_thread.is_alive()
        )
