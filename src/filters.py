...
def apply_3d_bilateral(
    ...
    """
    logging.info(
        f"Applying Bilateral filter with sigma_spatial={sigma_spatial}, sigma_color={sigma_color}..."
    )
...
def apply_3d_nlm(
    ...
    """
    logging.info(
        f"Applying NLM filter with h={h}, patch_size={patch_size}, patch_distance={patch_distance}..."
    )
...
def apply_3d_tv(
    volume: np.ndarray, weight: float, progress_callback=None, check_cancel=None
) -> np.ndarray:
    """
