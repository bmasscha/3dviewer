...
class HistogramWidget(QFrame):
    ...
    def paintEvent(self, event):
        ...
        if self.hist_data is not None and len(self.hist_data) > 0:
            ...
            painter.setBrush(
                QBrush(QColor(52, 152, 219, 150))
            )  # Blue-ish semi-transparent
...
class ImportDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        reduction_group.setStyleSheet(
            "background-color: #222; border-radius: 4px; padding: 5px;"
        )
        ...
        self.chk_load_all_channels = QCheckBox(
            "Load all channels for dynamic switching (Spectral CT)"
        )
        ...
        mem_group.setStyleSheet(
            "background-color: #1a2a33; border: 1px solid #34495e; border-radius: 4px; padding: 10px;"
        )
        mem_layout = QVBoxLayout(mem_group)
        self.mem_info_label = QLabel("Memory: Ready to estimate...")
        self.mem_info_label.setStyleSheet(
            "font-size: 13px; color: #ecf0f1; font-family: 'Consolas', 'Monaco', monospace;"
        )
    ...
    def on_browse_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select HDF5 Volume", "", "HDF5 Files (*.h5 *.hdf5);;All Files (*)"
        )
        if path:
            self.folder_path = path
            self.is_hdf5 = True
    ...
    def load_metadata(self):
        if self.is_hdf5:
            self.stats = self.core.volume_loader.get_h5_quick_stats(
                self.folder_path, self.channel_index
            )
        else:
        ...
        if self.stats:
            ...
            self.on_range_changed(
                self.hist_widget.lower_val, self.hist_widget.upper_val
            )
    ...
    def update_memory_info(self):
        ...
        if load_all:
            ...
            # Single channel
            is_safe, estimated, available = (
                self.core.volume_loader.check_memory_available(
        ...
        if is_safe:
            ...
            if hw_warning:
                ...
                self.mem_status_label.setText(
                    "Status: ❌ Critical: Memory Insufficient"
                )
    ...
    def update_preview(self):
        ...
        if self.is_hdf5:
            ...
            try:
                ...
                bytes_per_line = w
                q_img = QImage(
                    img_f.data, w, h, bytes_per_line, QImage.Format.Format_Grayscale8
                )
                pixmap = QPixmap.fromImage(q_img)
                self.preview_label.setPixmap(
                    pixmap.scaled(400, 400, Qt.AspectRatioMode.KeepAspectRatio)
    ...
    def load_all_channels(self):
        ...
        return (
            self.chk_load_all_channels.isChecked()
            and self.is_hdf5
            and self.stats
            and self.stats.get("num_channels", 1) > 1
        )
