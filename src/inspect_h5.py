        ...
        if "reconstruction" in f:
            ...
            if len(ds.shape) == 4:
                ...
                if ds.chunks:
                    ...
                    print(
                        f"    Chunk size: {np.prod(ds.chunks) * ds.dtype.itemsize / 1e6:.2f} MB"
                    )
                    print("    Chunks per dimension:")
                    print(
                        f"      Depth: {depth // chunk_d} chunks ({depth % chunk_d} remainder)"
                    )
                    print(
                        f"      Height: {height // chunk_h} chunks ({height % chunk_h} remainder)"
                    )
                    print(
                        f"      Width: {width // chunk_w} chunks ({width % chunk_w} remainder)"
                    )
                    print(
                        f"      Channels: {channels // chunk_c} chunks ({channels % chunk_c} remainder)"
                    )
                    ...
                    if chunk_c < channels:
                        ...
                        print(
                            f"    Chunks span multiple channels (chunk_c={chunk_c}, total={channels})"
                        )
                        print(
                            f"    Reading one channel requires reading {channels // chunk_c} times more data!"
                        )
                        print(
                            "    Recommendation: Load all channels at once, then split in memory"
                        )
                    else:
                        print(
                ...
                start = time.time()
                sample_data = ds[
                    mid_slice : mid_slice + 10, :, :, 0
                ]  # 10 slices, channel 0
                elapsed1 = time.time() - start
                print(f"    Read 10 slices of channel 0: {elapsed1:.3f}s")
                print(f"    Data shape: {sample_data.shape}")
                print(
                    f"    Estimated time for full channel: {elapsed1 * (depth / 10):.1f}s"
                )
                ...
                start = time.time()
                sample_all = ds[
                    mid_slice : mid_slice + 10, :, :, :
                ]  # 10 slices, all channels
                elapsed2 = time.time() - start
                print(f"    Read 10 slices, all {channels} channels: {elapsed2:.3f}s")
                print(f"    Data shape: {sample_all.shape}")
                print(
                    f"    Estimated time for all channels: {elapsed2 * (depth / 10):.1f}s"
                )
                print(
                    f"    Speedup vs sequential: {(elapsed1 * channels) / elapsed2:.1f}x faster"
                )
                ...
                print(
                    f"    Extract 2 channels from memory: {elapsed3:.6f}s (negligible)"
                )

                print("\n  RECOMMENDATION:")
                if ds.chunks and ds.chunks[3] < channels:
                    print(
                        "    ⚠️  Load entire 4D volume at once, then split channels in memory"
                    )
