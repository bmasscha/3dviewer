...
class AcquilaServer:
    ...
    def __init__(
        self, outbound_port=DEFAULT_OUTBOUND_PORT, inbound_port=DEFAULT_INBOUND_PORT
    ):
    ...
    def _setup_sockets(self):
        ...
        print(
            f"Acquila Server sockets bound on {self.inbound_port} (in) / {self.outbound_port} (out)"
        )
                            ...
                            if uuid_val:
                                with self.lock:
                                    if r_type == "SENT":
                                        ...
                                        print(
                                            f"[SERVER] Queueing: {data.get('command')} for {data.get('component')}"
                                        )

                                    elif r_type == "RCV":
                                        if uuid_val in self.command_queue:
                                            self.command_queue[uuid_val]["status"] = (
                                                "RUNNING"
                                            )
                                        ...
                                        if uuid_val in self.command_queue:
                                            print(
                                                f"[SERVER] Finished: {uuid_val} ({r_type})"
                                            )
                                            self.command_queue[uuid_val]["status"] = (
                                                "FINISHED"
                                            )
                                            self.command_queue[uuid_val][
                                                "reply type"
                                            ] = r_type
                                            self.command_queue[uuid_val]["reply"] = (
                                                data.get("reply", "")
                                            )
                                            self.command_queue[uuid_val][
                                                "finish_time"
...
class AcquilaClient:
    def __init__(
        ...
        print(
            f"[CLIENT] Connected to {server_ip}: {inbound_port}(in)/{outbound_port}(out)"
        )
    ...
    def send_command(
        self, component, command, arg1="", arg2="", wait_for="ACK", timeout_ms=10000
    ):
        """
        ...
        payload = self._create_payload(
            component, "", command, arg1, arg2, "", "SENT", my_uuid
        )
        ...
        while (time.time() * 1000 - start_time) < timeout_ms:
            try:
                ...
                if rec_json.get("UUID") == my_uuid:
                    ...
                    print(
                        f"   <-- Received response: {r_type} ('{rec_json.get('reply')}')"
                    )
    ...
    def send_command_until(
        self, component, command, expected_feedback, interval_ms=500, timeout_ms=30000
    ):
        ...
        while (time.time() * 1000 - start_time) < timeout_ms:
            response = self.send_command(
                component, command, wait_for="ACK", timeout_ms=2000
            )
            if response:
