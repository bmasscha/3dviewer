...
class CommandInterpreter:
    ...
    def set_provider(self, provider_type, model_name=None):
        ...
        if provider_type.lower() == "ollama":
            ...
            self.provider = llm_provider.GeminiProvider(
    ...
    def interpret(self, text, state=None):
        ...
        if any(
            w in text
            for w in ["help", "what can you do", "actions", "commands", "instructions"]
        ):
            ...
            if self.last_action:
                ...
                if action["action"] in absolute_actions:
                    if "params" in action and "value" in action["params"]:
                        ...
                        return (
                            action,
                            f"Increasing {action['action']} to {action['params']['value']:.2f}.",
                        )

                if (
                    modifier != 1.0
                    and "params" in action
                    and "value" in action["params"]
                ):
        ...
        return (
            None,
            "I'm sorry, I didn't quite catch that. Try 'zoom in' or ask 'help'.",
        )
    ...
    def _load_commands_file(self):
        ...
        try:
            commands_path = os.path.join(
                os.path.dirname(__file__), "documentation_3dviewer", "commands.md"
            )
    ...
    def _run_diagnostics(self):
        ...
        if isinstance(self.provider, llm_provider.OllamaProvider):
            ...
            return (
                None,
                f"Ollama is {status}. Active Model: <b>{self.provider.model_name}</b>. Available: {model_str}.",
            )
            ...
            return (
                None,
                f"Gemini Active. Model: <b>{self.provider.model_name}</b>. API Key: {api_key_status}. Available: {model_str}",
            )
    ...
    def _try_llm_parsing(self, text):
        ...
        options = {"temperature": 0.0, "num_predict": 128, "stop": ["User:", "\n\n"]}
        ...
        json_match = re.search(
            r'\{[^{}]*"action_dict"[^{}]*\}', response_text, re.DOTALL
        )
        if json_match:
            try:
        ...
        if isinstance(action, str):
            ...
            return {
                "error": f"<b>AI DATA ERROR:</b> Invalid structure ({type(action).__name__})"
            }

        # Success - log and update state
        self.history.append({"role": "user", "content": text})
        self.history.append(
            {"role": "assistant", "content": res_data.get("response", "")}
        )
        return res_data
    ...
    def _try_regex_parsing(self, text, raw_text):
        ...
        if "load" in text:
            path_match = re.search(
                r"load\s+(?:overlay\s+)?(.+)", raw_text, re.IGNORECASE
            )
            if path_match:
        ...
        if any(re.search(p, text) for p in zoom_in_phrases) or any(
            re.search(p, text) for p in zoom_out_phrases
        ):
            value = val_from_text if val_from_text is not None else 1.0
            if any(re.search(p, text) for p in zoom_out_phrases) and not any(
                re.search(p, text) for p in zoom_in_phrases
            ):
                value = -abs(value)
        ...
        # Rotate commands
        if any(
            re.search(r"\b" + w + r"\b", text)
            for w in ["rotate", "turn", "spin", "orbit"]
        ):
            axis = "y"
            if any(
                re.search(r"\b" + w + r"\b", text)
                for w in ["up", "down", "vertically", "slope"]
            ):
                axis = "x"
        ...
        if any(
            re.search(r"\b" + w + r"\b", text)
            for w in ["reset", "home", "default", "lost", "center", "start"]
        ):
            return {"action": "reset", "params": {}}

        # Rendering mode commands
        if any(
            re.search(r"\b" + w + r"\b", text) for w in ["mip", "maximum intensity"]
        ):
        ...
        if re.search(r"\bslice\b", text):
            ...
            if re.search(r"\bx\b", text):
                axis = "x"
            elif re.search(r"\by\b", text):
                axis = "y"
            elif re.search(r"\bz\b", text):
                axis = "z"

            if axis:
                if any(
                    re.search(r"\b" + w + r"\b", text) for w in ["middle", "center"]
                ):
                    return {
                        "action": "set_slice",
                        "params": {"axis": axis, "percent": 50.0},
                    }
                elif any(
                    re.search(r"\b" + w + r"\b", text) for w in ["start", "beginning"]
                ):
                    return {
                        "action": "set_slice",
                        "params": {"axis": axis, "percent": 0.0},
                    }
                elif re.search(r"\bend\b", text):
                    return {
                        "action": "set_slice",
                        "params": {"axis": axis, "percent": 100.0},
                    }
                elif val_from_text is not None:
                    if "%" in text or val_from_text <= 100:
                        return {
                            "action": "set_slice",
                            "params": {"axis": axis, "percent": val_from_text},
                        }
                    else:
                        return {
                            "action": "set_slice",
                            "params": {"axis": axis, "value": int(val_from_text)},
                        }
        ...
        if any(re.search(r"\b" + w + r"\b", text) for w in ["crop", "clip"]):
            ...
            if axis and len(nums) >= 2:
                ...
                return {
                    "action": "crop",
                    "params": {"axis": axis, "min": min(v1, v2), "max": max(v1, v2)},
                }
            elif any(
                re.search(r"\b" + w + r"\b", text) for w in ["half", "middle", "center"]
            ):
                if axis:
                    return {
                        "action": "crop",
                        "params": {"axis": axis, "min": 0.25, "max": 0.75},
                    }
        ...
        if re.search(r"\bthreshold\b", text):
            return {
                ...
                "params": {
                    "value": val_from_text if val_from_text is not None else 0.05
                },
            }
        ...
        if any(
            re.search(r"\b" + w + r"\b", text)
            for w in ["quality", "sampling", "resolution"]
        ):
            # If "increase" or "better", and no number, suggest 1.5x
            if (
                any(w in text for w in ["increase", "better", "higher", "more"])
                and val_from_text is None
            ):
                return {"action": "adjust_quality", "params": {"value": 1.5}}
            # If "decrease" or "lower", and no number, suggest 0.5x
            if (
                any(w in text for w in ["decrease", "lower", "faster", "less"])
                and val_from_text is None
            ):
        ...
        if any(re.search(r"\b" + w + r"\b", text) for w in ["density", "opacity"]):
            return {
                "action": "set_density",
                "params": {
                    "value": val_from_text if val_from_text is not None else 50.0
                },
            }
